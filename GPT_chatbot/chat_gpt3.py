...
# search for relavent conversation record with embedding
...
if __name__ == '__main__':
    ...
    conversation = ['USER: Hey V','V: Hey there! How can I help?']
    print('==============\n\V: Hey there! How can I help?')
    similar = list()
    while True:
        a = input('\nUSER: ')
        ...
        block += '\n\n'.join(conversation) + '\n'
        prompt = open_file('prompt_v.txt').replace('<<CONVERSATION>>', block)
        ...
        similar = similar_logs(vectors[0]) #search for relavent question before
